...
let proof tvs rmap =
  ...
  let proof oc p =
    ...
    | Pmp(k1,k2) -> string oc "⇒ₑ "; sub_at oc k1; char oc ' '; sub_at oc k2
    | Pdisch(t,k) ->
      string oc "⇒ᵢ (λ "; hyp_var ts oc t; string oc " : Prf "; term oc t;
      string oc ", "; sub_at oc k; char oc ')'
    | Pspec(t,k) -> string oc "∀ₑ "; sub_at oc k; char oc ' '; term oc t
    | Pgen(x,k) ->
       let rmap' = add_var rmap x in
       string oc "∀ᵢ (λ "; decl_var rmap' oc x; string oc ", ";
       subproof tvs rmap' [] [] ts k oc (proof_at k); char oc ')'
    | Pexists(p,t,k) ->
