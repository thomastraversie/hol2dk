...
let proof tvs rmap =
  ...
  let rec proof oc p =
    ...
    | Pmp(k1,k2) -> 
       ...
       let Proof(th,_) = p in
       let l,r = binop_args (concl th) in
       string oc "imp_elim "; term oc l; char oc ' '; term oc r; char oc ' '; 
       sub k1 oc p; char oc ' '; sub k2 oc (proof_at k2) 
    | Pdisch(t,k) -> 
      let p = proof_at k in
      let Proof(th,_) = p in
    ...
    | Pspec(t,k) -> 
      ...
      | Comb(_,m) -> string oc "all_elim "; typ oc (type_of t); char oc ' '; term oc m; char oc ' '; sub_at oc k; char oc ' '; term oc t
      | _ -> assert false
      end   
    | Pgen(x,k) ->
       let p = proof_at k in
       ...
       string oc "all_intro "; typ oc (type_of x); char oc ' '; term oc tt; string oc " ("; 
